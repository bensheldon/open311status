...
Fetcher.prototype = {
  ...
  pingRequests: function(pingCallback) {
    ...
    async.forEachSeries(Object.keys(endpoints), function(city, callback) {
      ...
      var params = {
        start_date: new Date(d - 60 * 60 * 1000).toString('yyyy-MM-ddTHH:mm:ss'),
        end_date: d.toString('yyyy-MM-ddTHH:mm:ss')
      };
        ...
        if (!err) {
          ...
          async.map(serviceRequests, function(serviceRequest, cb) {
            (new ServiceRequest({
              ...
              media_url          : serviceRequest.description.media_url,
  ...
  pingAll: function(){
    this.pingServices(function(servicesPings) {
      console.log('Pinged ' + servicesPings.length + ' services endpoints.');
    });
    this.pingRequests(function(requestsPings) {
